...
describe("stackSnapshot", () => {
  test("flattens the default stack and marks it as default", () => {
    ...
    expect(snapshot.framework).toBe("fastapi");
    expect(snapshot.addons).toBe("docker+ruff+pytest");
  ...
  test("marks customized stacks and joins multi-select lists", () => {
    ...
    expect(snapshot.framework).toBe("django");
    expect(snapshot.addons).toBe("ruff+mypy");
