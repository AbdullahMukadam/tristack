...
describe("agent discovery content", () => {
  test("puts the quick start first and advertises every supported automation interface", () => {
    ...
    expect(markdown).toStartWith("# TriStack");
...
describe("Markdown content negotiation", () => {
  ...
  test("preserves stack selections through the rewrite and Markdown route", async () => {
    const response = proxy(markdownRequest("/stack?name=agent-app&fw=fastapi&db=postgres"));
    const destination = `${SITE_URL}/agent-content/stack?name=agent-app&fw=fastapi&db=postgres`;
