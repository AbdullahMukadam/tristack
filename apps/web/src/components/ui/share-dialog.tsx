...
import { Check, Copy, Link2, QrCode, Share2, SquareTerminal } from "lucide-react";
...
function CopyRow({
  ...
  return (
    <button
      ...
      className="builder-focus-ring group flex w-full min-w-0 items-center gap-2 rounded-md border border-border bg-muted/10 px-3 py-2 text-left transition-colors hover:border-muted-foreground/30 hover:bg-muted/25"
    >
...
export function ShareDialog({ children, stackUrl, stackState, page }: ShareDialogProps) {
  ...
  const shareText = () => {
    ...
    return `${projectName} — my TriStack\n\n${summary}${rest}\n\n`;
  };
      ...
      <DialogContent className="grid max-h-[85vh] grid-cols-1 gap-3 overflow-y-auto bg-fd-background sm:max-w-lg">
        <DialogHeader className="border-border border-b pb-3">
          ...
          <DialogDescription className="text-xs text-muted-foreground">
            {projectName} · {selectedTechs.length} technology selection
        ...
        <div className={cn("grid gap-2", canNativeShare ? "grid-cols-3" : "grid-cols-2")}>
          <button
            ...
            className="builder-focus-ring flex items-center justify-center gap-1.5 rounded-md border border-border bg-muted/10 px-2 py-2 text-xs font-medium text-muted-foreground transition-colors hover:border-muted-foreground/30 hover:bg-muted/25 hover:text-foreground"
          >
          ...
          <button
            ...
            className={cn(
              "builder-focus-ring flex items-center justify-center gap-1.5 rounded-md border px-2 py-2 text-xs font-medium transition-colors",
              showQr
        ...
        {showQr && (
          <div className="flex flex-col items-center gap-2 rounded-md border border-border bg-muted/10 p-4">
            {qrCodeDataUrl ? (
              <Image
                src={qrCodeDataUrl}
                width={160}
              ...
              <div className="flex h-40 w-40 items-center justify-center text-xs text-destructive">
