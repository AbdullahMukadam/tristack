...
export function SelectedStackBadges({ stack, onRemove, onJump }: SelectedStackBadgesProps) {
  ...
  if (selections.length === 0) {
    return <p className="text-xs text-fd-muted-foreground">No selections yet</p>;
  }
      ...
      {selections.map(({ category, tech }) => {
        ...
        const chipContent = (
          <>
            ...
            {"experimental" in tech && tech.experimental && (
              <span className="rounded-full bg-amber-500/10 px-1.5 py-0.5 text-[9px] font-medium text-amber-700 leading-none dark:text-amber-300">
                Experimental
              </span>
            )}
          </>
        ...
        return (
          <span
            ...
            className={cn(
              "inline-flex items-center gap-1 rounded-md border pl-2 text-xs text-fd-foreground",
              onRemove ? "pr-1" : "pr-2",
            )}
          >
            {onJump ? (
