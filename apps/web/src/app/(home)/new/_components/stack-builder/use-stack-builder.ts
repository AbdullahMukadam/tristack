...
export function getTechSelectionUpdate(
  ...
  if (catKey === "addons") {
    ...
    const uniqueNext = [...new Set(nextArray)];
    const uniqueCurrent = [...new Set(currentArray)];
...
export function useStackBuilder() {
  ...
  useEffect(() => {
    const savedStack = localStorage.getItem("tristackStackPreference");
    if (!savedStack) {
      return;
    }
    ...
    try {
      ...
      localStorage.removeItem("tristackStackPreference");
    }
  }, []);
  ...
  function getRandomStack() {
    ...
    for (const category of CATEGORY_LIST) {
      const options = getOptionsForStack({ ...effectiveStack, ...randomStack }, category);
      if (options.length === 0) {
        continue;
      }
      ...
      if (catKey === "addons") {
        ...
        randomStack.addons = shuffledOptions.map((opt) => opt.id);
        continue;
      }
